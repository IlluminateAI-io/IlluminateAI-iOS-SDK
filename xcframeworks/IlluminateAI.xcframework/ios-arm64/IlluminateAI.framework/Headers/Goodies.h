...
+ (UIViewController*)VCFactory:(NSString*)class_;
...
template <typename T> 
inline const T pin(const T& _min, const T& _val, const T& _max) {
	return ((_val < _min) ? _min : ( (_val > _max) ? _max : _val));
}

template <typename T>
inline const Boolean between(const T& _min, const T& _val, const T& _max) {
